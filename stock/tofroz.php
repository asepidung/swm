...
// Menghitung tanggal 57 hari yang lalu
$date56DaysAgo = date('Y-m-d', strtotime('-56 days'));

$sql = "SELECT s.*, b.nmbarang, g.nmgrade
        FROM stock s
        JOIN barang b ON s.idbarang = b.idbarang
        JOIN grade g ON s.idgrade = g.idgrade
        WHERE s.idgrade IN (1, 3) AND s.pod <= '$date56DaysAgo'
        ORDER BY s.pod";
...
<script>
   ...
   document.title = "More Than 8 Weeks";
</script>
